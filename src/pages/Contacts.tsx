...
👉 Prends ta décision aujourd'hui et transforme ta vie avec nous !
    ...
    const downloadFile = async (url: string, filename: string = 'contacts.vcf') => {
        ...
        try {
            ...
            setFeedbackModal({ show: true, type: 'success', message: 'Téléchargement réussi!' });
        } catch (err) {
            setFeedbackModal({ show: true, type: 'error', message: 'Échec du téléchargement des contacts.' });
        } finally {
    ...
    const handleDownloadRange = async () => {
        if (!dateRange.from || !dateRange.to) {
            setFeedbackModal({ show: true, type: 'error', message: "Veuillez sélectionner les deux dates." });
            return;
        }
        const baseUrl = getBaseUrl();
                        ...
                        >
                            ...
                            >
                                <div className="text-center">
                                    <div className={`w-12 h-12 ${feedbackModal.type === 'success' ? 'bg-green-100' : 'bg-red-100'} rounded-full flex items-center justify-center mx-auto mb-4`}>
                                    ...
                                    <button
                                        onClick={() => setFeedbackModal({ ...feedbackModal, show: false })}
                                        className={`w-full ${feedbackModal.type === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white py-2 px-4 rounded-xl font-medium transition-colors`}
                                    >
                                        Fermer
                                    </button>
                                </div>
